...
export type SubtitleFile = {
  ...
  /** Every named cue written again after itself, as one undo step. */
...
export function useSubtitleFile(onRowsMoved: RowsMoved, onPanels: PanelSink): SubtitleFile {
  ...
  const duplicateCues = useCallback(
    (cues: number[]) => command("subtitle_duplicate", { cues }),
    [command],
  );

  const joinCues = useCallback(
    (cues: number[], keepFirstText: boolean) => command("subtitle_join", { cues, keepFirstText }),
    [command],
  );
