...
| `specs/chrome.spec.js`                  | `draws every title with nothing open, greying the one with nothing behind it`        | With no document and no media open: File, Edit, View, Audio and Help are all on the bar, Audio greyed because it has no track to list, and File draws Save, Save as and Discard greyed instead of leaving them out. Owner ruling 2026-09-03, which reverses decision 24 A2.                                                                                                                                                                                                                                                                     |
...
| `specs/command-registry.spec.js`        | `draws every command with nothing open, greyed rather than absent`                   | With no subtitle and no video: every title is on the bar, Audio greyed because no media has tracks; File draws Save, Save as and Discard, all greyed; Edit draws Undo and Redo, greyed; and the toolbar draws all seven of its buttons. Owner ruling 2026-09-03, which reverses decision 24 A2 (T3 C2).                                                                                                                                                                                                                                         |
| `specs/command-registry.spec.js`        | `refuses a greyed command from the menu, the toolbar and the keyboard`               | Greyed Undo, Redo and Save as are fired from the menu, from the toolbar, and Save as also from Ctrl+Shift+S. Nothing crosses the IPC boundary, counted with the `lib/ipc.js` probe, and no chooser is raised. The barrier: opening a subtitle through the toolbar straight after sends exactly one `subtitle_open`, so the zero is a refusal and not a probe that never took (T3 C2 and C3).                                                                                                                                                    |
| `specs/command-registry.spec.js`        | `greys and ungreys in place, with nothing appearing or disappearing`                 | Opening a subtitle leaves every title, every menu item and every toolbar button where it was, in the same order. The one greying that moves is Save as, and it moves on both routes at once (T3 C2).                                                                                                                                                                                                                                                                                                                                            |
...
| `scripts/close-gate-check.js`           | 13 checks                                                                            | Closing with unsaved edits asks save/discard/cancel; each answer is proved by the dialog going away, cancel keeps the app and the file, discard exits 0 leaving the file untouched, save writes the edit, moves nothing else and keeps one backup that holds the bytes it overwrote, under a name carrying the second it was taken (BACKLOG N1, N74).                                                                                                                                                                                           |
...
| `scripts/mpv-context-check.js`          | 6 checks                                                                             | A `gpu-context` mpv refuses costs the request and not the window: the app still starts, the refusal is in the log, the app says in its log that it fell back to the pinned `x11egl` and mpv attaches, and the video still attaches (BACKLOG N2b).                                                                                                                                                                                                                                                                                               |
...
| `scripts/memory-budget-check.js`        | 2 checks                                                                             | Idle memory with a video open and paused, summed as PSS across the app's process group, against CONTRIBUTING.md section 7's 400 MB. Leaves the webview workarounds armed, unlike every other harness here, because a memory number about a configuration no user gets is not the budget.                                                                                                                                                                                                                                                        |
| `scripts/cold-start-check.js`           | 2 checks                                                                             | Launch to the page's first command reaching the log, against CONTRIBUTING.md section 7's two seconds. A fresh data home every run, and the webview workarounds armed for the reason the memory check gives.                                                                                                                                                                                                                                                                                                                                     |
| `scripts/video-shutdown-check.js`       | 4 checks                                                                             | The close path stops mpv and takes the surface before the window goes, once per close and with or without a video open. The line that keeps N11 closed, which nothing else covered.                                                                                                                                                                                                                                                                                                                                                             |
| `scripts/writes-nowhere-else-check.js`  | 4 checks                                                                             | The app is given a home nobody has used, and writes nothing into it. CONTRIBUTING.md section 3's fifth rule, which nothing observed before. Names any stranger at the top of the XDG directories rather than ignoring it.                                                                                                                                                                                                                                                                                                                       |
...
| `specs/cue-structure.spec.js`           | `deletes the cue the cursor is on, from the menu`                                    | Subtitles > Delete cues takes the row the cursor is on out of the document, one `subtitle_delete_many` crosses carrying a set of one, and nothing is written (M2.7 E2).                                                                                                                                                                                                                                                                                                                                                                         |
...
`.toolbar__file-save-as`, `.toolbar__edit-undo`, `.toolbar__edit-redo`, `.toolbar__file-discard`, `.about`,
...
`.toolbar__file-open-subtitle`, and `.subbar__save`, `.subbar__undo`, `.subbar__redo` and
`.subbar__discard` are the same names under `.toolbar__`. `.subbar__save-copy` is
`.toolbar__file-save-as`, which is the command's own name since it stopped writing a copy. A menu item's id is
